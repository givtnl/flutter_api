...
{
    public class GetUserTagMatchesListQueryHandler : IRequestHandler<GetUserTagMatchesListQuery, IEnumerable<UserTagMatchListModel>>
    {
        private readonly IDynamoDBContext _dynamoDb;
        private readonly IMapper _mapper;

        public GetUserTagMatchesListQueryHandler(IDynamoDBContext dynamoDb, IMapper mapper)
        {
            _dynamoDb = dynamoDb;
            _mapper = mapper;
        }
        public async Task<IEnumerable<UserTagMatchListModel>> Handle(GetUserTagMatchesListQuery request, CancellationToken cancellationToken)
        {
            ...
            return response.Select(match => _mapper.Map(match, new UserTagMatchListModel())).ToList();
        }
    }
}